...
DROP INDEX IF EXISTS idx_name_tblMeasurementUnits;
CREATE UNIQUE INDEX idx_name_tblMeasurementUnits ON tblMeasurementUnits (name_rus);     
...
CREATE VIEW ViewMeasurementUnits AS 	
	SELECT 
		ID_tblMeasurementUnit AS 'id',
		m.name_rus AS 'название',
		m.short_name_rus AS 'сокр.',
		m.name_eng AS 'eng',
		m.short_name_eng AS 'short',
		--m.basis AS 'основной',
		m.multiplier AS 'множитель',
		tblPhysicalPropertys.name_rus AS 'физ.свойство',  
	CASE
	    WHEN m.basis > 0 THEN 'основной'
	    ELSE '--'
	END	AS 'подчиненность'
	FROM  tblMeasurementUnits m
	LEFT JOIN tblPhysicalPropertys ON tblPhysicalPropertys.ID_tblPhysicalProperty  = m.FK_tblPhysicalProperty_tblMeasurementUnits 
	
DROP VIEW ViewMeasurementUnits 
	
SELECT * FROM ViewMeasurementUnits
